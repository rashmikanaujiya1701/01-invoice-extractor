...
if uploaded_files:
    if st.button("Extract", type="primary"):
        ...
        m4.metric("Total GST", f"₹{df['gst_amount'].infer_objects(copy=False).fillna(0).sum():,.2f}")
        m5.metric("Total Value", f"₹{df['total_amount'].infer_objects(copy=False).fillna(0).sum():,.2f}")
        ...
        for fname, img, row in rows:
            with st.expander(f"{'⚠️' if row['needs_review'] else '✅'}  {fname}", expanded=False):
                ...
                c2.table(
                    pd.DataFrame(
                        {"Field": list(row.keys()), "Value": [str(v) if v is not None else "—" for v in row.values()]}
                    ).set_index("Field")
                )
